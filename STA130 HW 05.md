...
```python
# The null hypothesis is the statement of no effect, while the 
# alternative hypothesis suggests that an effect exists, guiding the 
...
```python
# The statement highlights that statistical tests aim to make
# conclusions about the population mean (𝜇), not just the sample mean 
# (𝑥𝑖). While 𝑥𝑖 estimates the sample's average, hypothesis testing
# evaluates whether this evidence supports a conslusion about the entire
# population mean (𝜇0). Thus, the test results refer to the population
# parameter, emphasizing that we are inferring about the larger group
# based on our sample findings.
```
...
```python
# Assuming the null hypothesis is true allows us to measure the
# strength of the evidence against it, helping us decide whether to 
# reject or fail to reject the null hypothesis based on the data.
```
...
```python
# A tiny p-value makes the null hypothesis seem "ridiculous" because it
# implies that it's very hard to believe the null hypothesis could still
# be true, given how extreme the actual data is. Thus, we are more likely
...
```python
# the simulated p-value is 0.0006, which means there is very strong 
# evidence against the null hypothesis (H0), according to the provided 
# table.

# This suggests that it's highly unlikely that head tilts when kissing
# are random (50/50 split). Instead, there seems to be a real tendency
# for couples to tilt their heads to the right.
```
...
```python
# a p-value van't definitively prove innocant or guilt, but it helps 
# assess the strength of evidence. There is no specific p-value that
# can provide absolute proof, only strong or weak evidence.
```
...
```python
# Code Change: Drop the 2 * in the p-value calculation and only look
# at the relevent tail (upper or lower).

# Interpretation: You are only testing for an effect in one direction,
# making the test more targeted.

# Smaller p-value: A one-tailed test will generally yield a smaller
# p-value compared to a two-tailed test, making it easer to reject the
...
# The original experiment involved a personalized test between two 
# individuals (Fisher and Bristol) where Bristol claimed she could 
# distinguish the taste when milk was poured first versus tea. Here, 
# the test involves a group of students in STA130, making it less 
# personalized. However, the statistical principles remain the same: 
# we are testing whether the students’ ability to differentiate is due 
# to random guessing or if it reflects a real ability.


# - Null Hypothesis (H₀): The STA130 students are guessing randomly 
# and have no true ability to differentiate which was poured first.

# - Formal:  H_0: p = 0.5 , where  p  is the probability of correctly 
# identifying whether tea or milk was poured first.

# - Informal: Students cannot actually tell the difference, and any 
# correct answers are due to random guessing.

# - Alternative Hypothesis (H₁): The STA130 students have a real
# ability to correctly identify which was poured first.

# - Formal:  H_1: p > 0.5 , suggesting students are better than random
# guessing.
```
...
# - Simulate random guessing (50/50 probability) across 80 students.
# - Calculate how often 49 or more students guess correctly.
# - Compare this result to the observed data to estimate the p-value.
...
# Data
n_students = 80
correct_responses = 49
null_probability = 0.5

# Simulations
n_simulations = 10000  # Number of simulations to perform
simulations = np.random.binomial(n=n_students, p=null_probability, size=n_simulations)

# Calculate p-value: proportion of simulations where correct guesses >= 49
p_value = np.mean(simulations >= correct_responses)
...
# since the p-value is below the standard significance level of 0.05, we
# reject the null hypothesis. This suggests that the STA130 students 
# likely have a genuine ability to distinguish whether milk or tea was 
# poured first, and their performance is not due to random guessing
```
...
```python
# yessir
# didn't use chatbot for this week's homework
```
